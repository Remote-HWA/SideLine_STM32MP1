...
static void CipherSub(state_t* state, const uint8_t* RoundKey)
{
  uint32_t val,val1,val2 = 0;
  // Add the First round key to the state before starting the rounds.
...
void AES_ECB_encrypt_Sub(const struct AES_ctx* ctx, uint8_t* buf)
{
	  // The next function call encrypts the PlainText with the Key using AES algorithm.
	  //CipherSub((state_t*)buf, ctx->RoundKey);
	  //uint32_t val,val1,val2 = 0;
	  // Add the First round key to the state before starting the rounds.
	  //asm volatile ("MRC p15, 0, %0, c9, c13, 0\t\n": "=r"(val));
	  AddRoundKey(0, (state_t*)buf, ctx->RoundKey);
	  //asm volatile ("MRC p15, 0, %0, c9, c13, 0\t\n": "=r"(val1));
	  SubBytes((state_t*)buf);
	  //asm volatile ("MRC p15, 0, %0, c9, c13, 0\t\n": "=r"(val2));

	  /*xil_printf("Before ARK : %d\n\r",val);
	  xil_printf("ARK : %d\n\r",val1-val);
	  xil_printf("After ARK : %d\n\r",val1);
	  xil_printf("SBOX : %d\n\r",val2-val1);
	  xil_printf("After SBOX : %d\n\r",val2);*/
}
